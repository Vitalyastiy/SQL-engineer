...
--==Абоненты ответившие на второй шаг (ANS_2) и не ставшие отвечать дальше
Дома (постоянное проживание)                    Indoor в этажной застройке
На работе\в школе\университете                  На улице/за городом
По маршруту движения\на дороге (трассе)         На улице/за городом
За городом\на даче (временное пребывание)       Частный сектор
Места отдыха и развлечений                      На улице/за городом
...
inner join prd2_odw_v.smspoll_poll_lists b on a.sub_list_id = b.sub_list_id
 and b.created_date >= timestamp '2023-03-02 00:00:00'
 and b.created_date < timestamp '2023-03-03 00:00:00'
 and b.poll_id in (141)
where 1=1
 and a.created_date >= timestamp '2023-03-02 00:00:00'
 and a.created_date < timestamp '2023-03-03 00:00:00'
group by 1,2
order by 1
;
...
where 1=1
 and poll_id in (141)
 and created_dttm >= timestamp '2023-03-02 00:00:00'
 and created_dttm < timestamp '2023-03-03 00:00:00'
group by 1,2
;
...
REPLACE PROCEDURE uat_ca.mc_nps_bu_141 (in stime timestamp, in etime timestamp)
...
SET proc = 'nps_mi_2022';           -- наименование расчета
...
CALL uat_ca.prc_debug (proc,:load_id,session,0,'START: ' || to_char(141));
...
CALL uat_ca.prc_debug (proc,:load_id,session,null,'INTERVAL_DATE '|| to_char(cast(stime as date),'DD.MM.YYYY') || '_' ||to_char(cast(etime as date),'DD.MM.YYYY') || ' Всего строк: ' || to_char(141) || ' in - ' || trim(cast(row_cnt as VARCHAR(4000) CHARACTER SET UNICODE NOT CASESPECIFIC)));
...
--select * from soa_2 where msisdn = '79026198531';
...
select
 ...
 case when a.ans_2 is null then null
      ...
      when a.ans_2 = 4 then 'По маршруту движения\на дороге'
...
select
 ...
 case when a.ans_5 is null then null
      when a.ans_5 = 1 then 'Дорога внутри города'
      when a.ans_5 = 2 then 'Дорога в соседние населенные пункты\на дачу'
      when a.ans_5 = 3 then 'Дорога в соседние регионы (М4, М5 и т.д.)'
...
select
 ...
 case when mark_5 is null then null
      ...
      then case when regexp_substr(oreplace(oreplace(oreplace(upper(mark_5),' ',''),',',''),'.',''), '[1-9]\d{0,2}') in ('1', '2', '3', '4')
...
--абоненты с долетами
insert into uat_ca.poll_id_141
select
 a.create_date,
 a.msisdn,
 a.subs_id,
...
CALL uat_ca.prc_debug (proc,:load_id,session,null,'Строк по долетам: ' || to_char(141) || ' fin - ' || trim(cast(row_cnt as VARCHAR(4000) CHARACTER SET UNICODE NOT CASESPECIFIC)));
...
CALL uat_ca.prc_debug (proc,:load_id,session,null,'Строк без долетов: ' || to_char(141) || ' fin - ' || trim(cast(row_cnt as VARCHAR(4000) CHARACTER SET UNICODE NOT CASESPECIFIC)));
...
CALL uat_ca.prc_debug (proc,:load_id,session,1,'END: ' || to_char(141));
...
-- Комментарий
COMMENT ON PROCEDURE uat_ca.mc_nps_bu_141 AS
'Процедура формирования витрины с опросами NPS SOA 2.0, тк Мобильный интернет 2022. Результирующая таблица: uat_ca.poll_id_141';
...
--2022
/*week 36*/ call uat_ca.mc_nps_bu_141 (timestamp '2022-09-06 00:00:00', timestamp '2022-09-07 00:00:00');   -- 2 мин. 53 сек.
...
where t1.ProcessName = 'nps_mi_2022'
...
--Статистика
COLLECT SUMMARY STATISTICS ON uat_ca.poll_id_141;

COLLECT STATISTICS
 COLUMN (CREATE_DATE),
 COLUMN (SUBS_ID),
 COLUMN (MSISDN)
ON uat_ca.poll_id_141;


-- Просмотр витрин
select top 100 * from uat_ca.poll_id_141;
select top 100 * from uat_ca.poll_id_141 where create_date >= date '2022-09-05';

--delete uat_ca.poll_id_141 where trunc (create_date,'iw') = date '2022-09-05';


select create_date, count(distinct msisdn), count(*) from uat_ca.poll_id_141 group by 1 order by 1 desc;
select trunc (create_date,'iw') as week_first_day, weeknumber_of_year (create_date, 'ISO') as week_num, count(distinct msisdn), count(*) from uat_ca.poll_id_141 group by 1,2 order by 1 desc;
...
--View
replace view uat_ca.v_poll_mi_2 as
lock row for access
select
 a.create_date,
...
--==Горно Алтайск
select * from prd2_dic_v.branch;        171, Горно-Алтайск
select * from prd2_dic_v.region;        16, Горно-Алтайск

select * from uat_ca.v_poll_mi_2 where region = 'Горно-Алтайск';
...
select top 100 * from uat_ca.v_poll_mi_2;
select * from uat_ca.v_poll_mi_2 where region is null;

select * from uat_ca.v_poll_mi_2 sample 25;
...
select * from uat_ca.v_poll_mi_2;
...
create multiset table uat_ca.poll_id_141 (
 create_date date format 'yy/mm/dd',
 msisdn varchar(20) character set unicode not casespecific,
 subs_id decimal(12,0),
 treatment_code varchar(9) character set unicode not casespecific,
