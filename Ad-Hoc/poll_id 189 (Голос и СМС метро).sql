...
--2023
/*week 07*/ call uat_ca.mc_nps_bu_189 (timestamp '2023-02-13 00:00:00', timestamp '2023-02-17 00:00:00');   -- 1 мин. 53 сек.
...
where t1.ProcessName = 'nps_189'
 and t1.logdate > current_timestamp(0) - interval '1' month
order by logdate desc;


select top 100 * from uat_ca.poll_id_189;
select top 100 * from uat_ca.v_poll_189;

--delete uat_ca.poll_id_189;
...
--189   NPS_Голос_и_СМС_2023_метро
...
REPLACE PROCEDURE uat_ca.mc_nps_bu_189 (in stime timestamp, in etime timestamp)
...
SET proc = 'nps_189';           -- наименование расчета
...
select
 ...
 case when a.ans_3 is null then null
      ...
      when a.ans_3 = 3 then 'По маршруту движения\на дороге\в метро'
...
select
 ...
 case when a.ans_6 is null then null
      when a.ans_6 = 1 then 'В метро'
      when a.ans_6 = 2 then 'Дорога внутри города'
      when a.ans_6 = 3 then 'Дорога в соседние населенные пункты\на дачу'
      when a.ans_6 = 4 then 'Дорога в соседние регионы (М4, М5 и т.д.)'
      when a.ans_6 = 5 then 'Другое'
...
select
 ...
 case when mark_6 is null then null
      ...
      then case when regexp_substr(oreplace(oreplace(oreplace(upper(mark_6),' ',''),',',''),'.',''), '[1-9]\d{0,2}') in ('1', '2', '3', '4', '5')
...
--абоненты с долетами
insert into uat_ca.poll_id_189
select
 a.create_date,
 a.msisdn,
 a.subs_id,
...
-- Комментарий
COMMENT ON PROCEDURE uat_ca.mc_nps_bu_189 AS
'Процедура формирования витрины с опросами NPS SOA 2.0, тк Голос и СМС. Результирующая таблица: uat_ca.poll_id_189';
...
--View
replace view uat_ca.v_poll_189 as
lock row for access
select
 a.create_date,
...
--show table uat_ca.poll_id_189;

create multiset table uat_ca.poll_id_189 (
 create_date date format 'yy/mm/dd',
 msisdn varchar(20) character set unicode not casespecific,
 subs_id decimal(12,0),
 treatment_code varchar(9) character set unicode not casespecific,
