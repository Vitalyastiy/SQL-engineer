...
where sector_name = 'RO1819_1'





--Вариант 2(другая скоринговая модель) 
--select top 100 * from uat_ca.mc_mc_sqm_score_2

create multiset table uat_ca.vf_rostov_research_17052023 as(


select t1.subs_id, t1.sector_name, bs_position, t1.coeff_80, t1.top_3
...
---------------------
select top 100 * 
from uat_ca.vf_rostov_research_17052023


---Step 2
create multiset table uat_ca.vf_rostov_research_17052023_hwe as(
select
 case
...
from uat_ca.vf_rostov_research_17052023 t1 
...
select top 100 * from uat_ca.vf_rostov_research_17052023_hwe



select  top 100 * from uat_ca.vf_rostov_research_17052023
...
--Вариант 1
--select top 100 * from uat_ca.mc_mc_sqm_score
-----------------------------------------------
create multiset table uat_ca.vf_rostov_research_052023 as(
select t1.subs_id, t1.sector_name, bs_position, t1.coeff_80, t1.top_3
	 , case when coeff_80 is null then 'not_bad_coeff'
	 	    when coeff_80 = 0 then 'not_bad_coeff'
	 	    else coeff_80 end as coeff_80_new
...
---------------------
select count(*) 
from uat_ca.vf_rostov_research_052023
-------------------------


---Step 2
create multiset table uat_ca.vf_rostov_research_052023_hwe as(
select
 case
  WHEN SUBSTR(SECTOR_NAME,LENGTH(SECTOR_NAME),1) IN ('4','7') THEN SUBSTR(SECTOR_NAME, 1, LENGTH(SECTOR_NAME)-1)||'1'
  WHEN SUBSTR(SECTOR_NAME,LENGTH(SECTOR_NAME)-2,3) IN ('021','011','081','041') THEN SUBSTR(SECTOR_NAME, 1, LENGTH(SECTOR_NAME)-3)||'1'
...
from uat_ca.vf_rostov_research_052023 t1 
left join  prd2_sec_v.hwe t2 on t1.subs_id = t2.subs_id  
   						    and t2.report_date = '2023-03-01') 
with data
primary index (subs_id);
...
create multiset table uat_ca.vf_rostov_research_18052023_hwe_all as(
select
 case
  WHEN SUBSTR(SECTOR_NAME,LENGTH(SECTOR_NAME),1) IN ('4','7') THEN SUBSTR(SECTOR_NAME, 1, LENGTH(SECTOR_NAME)-1)||'1'
  WHEN SUBSTR(SECTOR_NAME,LENGTH(SECTOR_NAME)-2,3) IN ('021','011','081','041') THEN SUBSTR(SECTOR_NAME, 1, LENGTH(SECTOR_NAME)-3)||'1'
